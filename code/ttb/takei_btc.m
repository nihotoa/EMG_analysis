...
fig   =figure('Name','T-controller',...
   ...
   'Units','pixels',...
   'UserData',[],...
   'Position',[0 37 448 903]);%[-922 37 448 903]
...
h1  = uicontrol(fig,'BackgroundColor',[1 1 1],...
   ...
   'Min',1,...
   'String','',...
   'Callback','s=get(gcf,''UserData'');ind = get(gco,''Value''); filenames   = get(gco,''String'');s.selectedfile =filenames{ind}; triggerind = get(s.h.settrigger,''Value''); triggernames   = get(s.h.settrigger,''String'');s.selectedtrigger =triggernames{triggerind};takei(s.viewerfig,[s.parentpath,''\'',s.selectedfile],s.selectedtrigger);set(gcf,''UserData'',s)');


uicontrol(fig,'BackgroundColor',get(fig,'Color'),...
   'Style','pushbutton',...
   'Callback','s=get(gcf,''UserData'');s.parentpath=uigetdir(s.parentpath);temp    = what(s.parentpath);s.filenames   = temp.mat;set(s.h.filelist,''String'',s.filenames,''Value'',1);set(s.h.textbox,''String'',s.parentpath);set(gcf,''UserData'',s)',...
   'HorizontalAlignment','center',...
   'Units','normalized',...
