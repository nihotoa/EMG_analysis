        ...
        try
            ...
            if(any(ismember(contents,'xcorr')))
                ...
                for iTar    = 1:nTar
                    ...
                    [temp,xcorrRmaxind] = max(abs(S.xcorr(iTar,:)));
                    S.xcorrRmax(iTar)   = S.xcorr(iTar,xcorrRmaxind);
                    S.xcorrRmaxlag(iTar)= S.xcorrlag(xcorrRmaxind);
                    S.xcorrRmaxP(iTar)  = S.xcorrP(iTar,xcorrRmaxind);
                    S.xcorrRmaxH(iTar)  = S.xcorrH(iTar,xcorrRmaxind);
                end
                disp('xcorr')
            end
