...
%plot & save settings
plk = 3; %set plot kinds(1:center trig plot, 2:trig to trig, 3:each nomalize plot)

%center trig plot
trig = 5; %1:plot obj1 start timing, 
...
rangeF_ms = 100; %how long do you want to see former part of trigger[msec] 
rangeL_ms = 200; %how long do you want to see latter part of trigger[msec]
...
%           CStimMarker(1,:) = CStimMarker(1,:) - TimeRange(1)*SampleRate_CStimMarker;
          ...
          x =  rangeF + 1;
            ...
            for j = 1:EMG_num
                ...
                plot([(rangeF + 1) (rangeF + 1)],[0 100],'y');
                hold on;
                plot(ave,'r','LineWidth',2);
                ylim([0 40]);
...
%     x = linspace(0,sum(Tim_range_ave)*1000/SampleRate,sum(Tim_range_ave)-2);
    x = linspace(0,100,sum(Tim_range_ave)-2);   
    for j = 1:EMG_num   
        for k = 1:3
        ...
        plot([Tim_range_ave_alt(1)*100/(10*sum(Tim_range_ave)-2) Tim_range_ave_alt(1)*100/(10*sum(Tim_range_ave)-2)],[0 100],'y','LineWidth',1.5);
        hold on;
%         plot([Tim_range_ave_alt(1)+Tim_range_ave_alt(2)-1*1000/SampleRate Tim_range_ave_alt(1)+Tim_range_ave_alt(2)-1*1000/SampleRate],[0 100],'y','LineWidth',1.5);
        plot([(Tim_range_ave_alt(1)+Tim_range_ave_alt(2)-1*1000/SampleRate)*100/(10*sum(Tim_range_ave)-2) (Tim_range_ave_alt(1)+Tim_range_ave_alt(2)-1*1000/SampleRate)*100/(10*sum(Tim_range_ave)-2)],[0 100],'y','LineWidth',1.5);
        hold on;
        plot(x,mean(all_pullData),'r','LineWidth',1.5);
        ylim([0 70]);
%         xlim([0 sum(Tim_range_ave_alt)-2*1000/SampleRate]);
        xlim([0 100]);
        hold on;
        xlabel('task[%]') % x-axis label
