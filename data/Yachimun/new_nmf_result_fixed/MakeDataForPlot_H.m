...
%    TimSUC = load([Allfiles{J} '_SUC_Timing.mat']);
   synergyData = load([Allfiles{J} '_12fix_nmf.mat'],'test');
   Hdata = synergyData.test.H;
   Wdata = synergyData.test.W;
   cd ../
  
   altH = [Hdata{synNum,1} Hdata{synNum,2}(K.k(1,:)',:) Hdata{synNum,3}(K.k(2,:)',:) Hdata{synNum,4}(K.k(3,:)',:)];
   altW = (Wdata{synNum,1} + Wdata{synNum,2}(:,K.k(1,:)') + Wdata{synNum,3}(:,K.k(2,:)') + Wdata{synNum,4}(:,K.k(3,:)'))./synNum;
   
   allH{J} = altH(OD.k_arr(:,find(OD.days==str2num(AllDays{J}))),:);
   allW{J} = altW(:,OD.k_arr(:,find(OD.days==str2num(AllDays{J}))));
end

 ResAVE.tData1_AVE = cell(1,synNum);
 ResAVE.tData2_AVE = cell(1,synNum);
 ResAVE.tData3_AVE = cell(1,synNum);
...
for SS = 1:S(2) %session loop
   ...
   cd ../../../MATLAB/data/Yachimun/new_nmf_result_fixed
   ...
   [alignedData, alignedDataAVE,AllT,Timing_ave,TIME_W] = alignData(allH{SS}, tim, ts(1),pre_per,post_per, synNum);
   taskRange = [-1*pre_per, 100+post_per];
   [Res] = alignDataEX(alignedData, tim, D, pre_per,post_per,TIME_W, synNum);
   ...
   if save_data == 1
       ...
       xpdate = AllDays(SS);
       cd 'synData'
       save([monkeyname 'Synfixed' AllDays{SS} '_Pdata.mat'], 'monkeyname','xpdate','D',...
