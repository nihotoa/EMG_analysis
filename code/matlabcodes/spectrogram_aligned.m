...
% Filter 
PreprocessECoG.fFilter = 1;% ?t?B???^???|????????1?A????????????0
PreprocessECoG.FilterN = 2;% ?|?????t?B???^??????
PreprocessECoG.FilterWn = [10 240];%?J?b?g?I?t???g??
PreprocessECoG.FilterType = 'bandpass';%?t?B???^??????
PreprocessECoG.SampFreq = 1350 ;% ?????M?????T???v?????O???g??

        
% load your file and variable        
load('Wasa_Wa181112_holdOnset_M1_2');
...
for k = 1:32 %
    ...
    S = S';
    [S_tate S_yoko] = size(S);                          %P_tate ... freq????, P_yoko ... time????
