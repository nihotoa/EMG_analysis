function hpol = polarfill(cax,theta,rho,line_style,TTickLabel)
    %POLARFILL  Polar coordinate plot (Fill).
...
%         end
        ...
        TTickLabel  = {TTickLabel{2:end},TTickLabel{1}};
        for i = 1 : length(th)
            text(rt * cst(i), rt * snt(i), TTickLabel{i},...
                'HorizontalAlignment', 'center', ...
                'HandleVisibility', 'on', 'Parent', cax);
            
            text(-rt * cst(i), -rt * snt(i), TTickLabel{i+length(th)},...
                'HorizontalAlignment', 'center', ...
                'HandleVisibility', 'on', 'Parent', cax);
        end
...
%         q = plot(xx, yy, 'Parent', cax);
        q = fill(xx, yy,'b', 'Parent', cax);
    else
%         q = plot(xx, yy, line_style, 'Parent', cax);
        q = fill(xx, yy, line_style, 'Parent', cax);
    end
