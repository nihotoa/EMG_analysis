...
% modified by Tomohiko Takei 20110202 referring http://www.mathworks.com/matlabcentral/newsreader/view_thread/289064
...
switch option
    ...
    case 'y'
        ...
        KEY = 'graphics_linkaxes_y';
        for i=1:length(ax)
            setappdata(ax(i),KEY,hlink);
        end
    case 'xy'
        set(ax,'XLimMode','manual','YLimMode','manual');
        hlink_X = linkprop(ax,'XLim');
        KEY_X   = 'graphics_linkaxes_x';
        hlink_Y = linkprop(ax,'YLim');
        KEY_Y   = 'graphics_linkaxes_y';
        
        for i=1:length(ax)
            setappdata(ax(i),KEY_X,hlink_X);
            setappdata(ax(i),KEY_Y,hlink_Y);
        end
    ...
    case 'off'
        ...
        KEY_Y   = 'graphics_linkaxes_y';
        for i=1:length(ax)
            setappdata(ax(i),KEY_X,hlink);
            setappdata(ax(i),KEY_Y,hlink);
        end
