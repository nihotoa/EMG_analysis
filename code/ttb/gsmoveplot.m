function gsmoveplot(h,opt,tool)
% function gsmoveplot(h,option)
%
...
% >>
%
% If the function is invoked on a line which already uses the gsmoveplot function, then the
...
% And test for double-click to end gsmoveplot action
...
if nargin<3  % i.e. user-invoked.  
    b=get(h,'tag');
    if ~isempty(b)&strcmp(b,'gsmoveplot')&(nargin==1|strcmp(opt,'off'))  % function being de-invoked
        k=get(h,'userdata');
        ...
        if ~isempty(get(h,'zdata'))
            error('gsmoveplot only works for 2-D graphs')
        end
        if strcmp(opt,'a')
            opt='axy';
        end
        ...
        set(h,'buttondownfcn',['gsmoveplot(gco,''' opt ''',1)'],'userdata',k,'tag','gsmoveplot');
    ...
    if strcmp(get(gcf,'selectiontype'),'open')
        ...
        case 1 % line's buttondownfcn invoked
            ...
            k=get(h,'userdata');
            k.initialcp = cp;
            k.initialx  = get(h,'xdata');
            k.initialy  = get(h,'ydata');
            ...
            set(h,'userdata',k);
            set(gcf,'windowbuttonmotionfcn',['gsmoveplot(get(gca,''userdata''),''' opt ''',2)'],'doublebuffer','on');
            set(gca,'userdata',h);
            set(gcf,'windowbuttonupfcn',['gsmoveplot(get(gca,''userdata''),''' opt ''',3)']);
        ...
        case 2 % button motion function
            ...
            switch opt
                ...
                case 'y'
                    if(k.index == 3)
                        diffcp   = cp  - k.initialcp;
                        x   = k.initialx + diffcp(1,1);
                        y   = k.initialy + diffcp(2,2);
                    else
                        y(k.index)=cp(2,2);
                    end
                case {'xy','axy'}
...
function redefine_tawindow(h)
global gsobj

scopeXData  = gsobj.scope.XData;
x   = get(h,'XData');
y   = get(h,'YData');
x   = nearest(scopeXData,x(1));
...
eval(['gsobj.tawindow.',tawindow_type,'(',num2str(tawindow_ind),').XData=x;']);
eval(['gsobj.tawindow.',tawindow_type,'(',num2str(tawindow_ind),').YData=y;']);
