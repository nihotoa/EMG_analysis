...
allH = cell(S);
cd order_tim_list
[fName,pat] = uigetfile(['*_' sprintf('%d',Tsynergy) '.mat']);
cd(pat)
OD = load(fName);
cd ../../
for J =1:S(2)     %session loop
   cd([Allfiles{J} '_' task '/' Allfiles{J} '_syn_result_' sprintf('%02d',EMG_num) '/' Allfiles{J} '_H/'])
   K = load([Allfiles{J} '_aveH3_' sprintf('%d',Tsynergy) '.mat'],'k');
   cd ../../
%    TimSUC = load([Allfiles{J} '_SUC_Timing.mat']);
   synergyData = load([Allfiles{J} '_' task '_' sprintf('%02d',EMG_num) '_nmf.mat'],'test');
...
for SS = 1:S(2) %session loop
   cd(['../easyData/' Allfiles{SS} '_' task])
