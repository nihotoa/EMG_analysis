function [Model, Info] = linear_sparse_space(X,Y,Model,parm)
...
fprintf('Linear sparse space start\n')
...
% 'linear_sparse_space'
%    ARD term = alpha * W^2 /SY  : A = W^2/SY
...
%    H(k)   = 0.5*( log_sw + log_a - WWA );
	...
	% E = ( (Y-W*X)^2 +  W^2 * (1/A) )/SX
    % Weight variance
    % SW = inv( XX + diag(Ainv)/T );   ( M x M )
