%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%{
時間シナジーを、トリミングするための関数.
トリミング範囲などは,パーセントで指定することができる
トリミング結果などはsynDataフォルダの中に~_Pdata.matといった要領で保存される
参照している.matファイルをはじめ、どの関数で作られたかわからないデータがインポートされているので、原因究明に努める
%}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%↓easy_data → _standard.matファイルを選択する
...
allH = cell(S);
cd order_tim_list
OD = load('orderSyn_second.mat'); %このファイルはどこで作られた？
cd ../
for J =1:S(2)     %session loop
   cd([Allfiles{J} '/' Allfiles{J} '_syn_result_' num2str(EMG_num) '/' Allfiles{J} '_H/'])
   K = load([Allfiles{J} '_aveH3_4.mat'],'k');
   cd ../../
%    TimSUC = load([Allfiles{J} '_SUC_Timing.mat']);
   synergyData = load([Allfiles{J} '_' num2str(EMG_num) '_nmf.mat'],'test');
...
D.obj1_per = [50 50]; %タイミング1の前後何%をトリミング？
D.obj2_per = [50 50]; %タイミング2の前後何%をトリミング?
D.task_per = [25,105]; %タスク全体の前後何%をトリミング?
for SS = 1:S(2) %session loop
    %↓Yachimun → easyData → Ya170630_standardの中にある_EasyData.matを参照するように変更する
   cd ../
   ...
   [Res] = alignDataEX(alignedData, tim, D, pre_per,post_per,TIME_W, Tsynergy); %変数「Res」のデータをプロットしたり、保存したりしているので、この変数に注目する
   ...
   %↓各タイミング付近でトリミングした時間シナジーをプロットしていく(どこでタイミング入ったかを図示していない)
   %4つシナジーがあるのに、各タイミングでシナジー１しかプロットしていない→やっつけで作った関数で、ちゃんとした図示の関数は他に用意されているのでは？
   switch PLOTF
       case 'on'
    %    if SS == 1
...
TIME_W = round(sum(Timing(:,end)-Timing(:,2) + 1)/s_num); %タスクにかかる平均時間(サンプリングレートは不明)
...
AllT = pre1_TIME+TIME_W+post2_TIME; %タスク前(割合指定) + タスク(100%) + タスク後(割合指定)
%j:muscle number 
%i:trial number
...
%↓各タスクを時間正規化する(平均時間であるTIME_Wに合わせてから、リサンプリング)
for j = 1:EMG_num %EMG_numになっているが、実際は筋シナジーの数であることに注意(変数名変えるとややこしくなりそうだから変えてない)
    DataA = zeros(s_num,AllT); %タスク数 × 平均時間(平均フレーム)
    for i = 1:s_num
%         trialData{i,2} = Data(j,Timing(i,2):Timing(i,5));???
         time_w = round(Timing(i,5) - Timing(i,2) +1); %タスクにかかる時間
        if time_w == TIME_W
            sec(1,1) = sec(1,1)+1;
...
for m = 1:EMG_num
        ...
        tD2 = cell(L,1);
        tD3 = cell(L,1);
    for i = 1:L%Trial loop(タスク回数だけループ)
        TimingPer(i,:) = Timing(i,:)./Timing(i,5); %タスク間を100%としたときに、各セクションでかかる時間
        ...
        tD1{i,1} = D{1,m}(i,centerP1(i,1):centerP1(i,2)); %時間シナジーからトリミング(tim1まわり)(範囲は指定したrangeに依存)
        tD2{i,1} = D{1,m}(i,centerP2(i,1):centerP2(i,2)); %tim2周り
        tD3{i,1} = D{1,m}(i,centerP3(i,1):centerP3(i,2)); %task_start周り
