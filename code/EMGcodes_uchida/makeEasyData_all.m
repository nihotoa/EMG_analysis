...
%% down data
downsample = 1; %0/1 means no/yes
downdata_to =11000; %recording frequency[Hz]
if save_E == 1
%     downsample = 1;
    downdata_to = 5000; %sampling frequency [Hz]
    if downsample==1
%         AllData_ECoG = resample(AllData_ECoG,downdata_to,ECoG_Hz);
...
Si = size(Tp);
% CTcheck = struct('data0',cell(Si(1),1),...
%                  'data1',cell(Si(1),1),...
%                  'data2',cell(Si(1),1),...
%                  'data3',cell(Si(1),1)...
%                  );
...
%         
%     case 'ECoG_syn'
%          load([monkeyname xpdate '-0002'],'CLFP_001*');
%          cd(save_fold)
%          cd([monkeyname xpdate]) 
%              for i = 1:ECoG_num
...
%             load([monkeyname xpdate '-' sprintf('%04d',i) '.mat'],['CLFP_0' sprintf('%02d',j)]);
            if get_first_data
                S1 = load([monkeyname xpdate '-' sprintf('%04d',i)],'CLFP_001*');
%                 TimeRange = zeros(1,2);
%                 TimeRange(1,1) = S1.CLFP_001_TimeBegin;
...
% Semg = load([monkeyname xpdate '-' sprintf('%04d', file_num(1,1)) '.mat'], 'CEMG_001Time*');
...
dt3s = cell(Sp(1),1);
    for j = 1:L
        ...
        f = pullData(j,:);
        Ds = diff(f)/h;   % first derivative
        dt1s{j} = downsample(Ds,10);
        Ds = diff(Ds)/h;   % second derivative
