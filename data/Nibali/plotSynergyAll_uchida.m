...
if kf > 1
    ...
    for ii = 1:pcNum
        ...
        switch synergy_type
            ...
            case 'TimeNormalized_task'
               ...
               load('task_normalized_hist.mat','tim_hist');
               [tim_hist_num,~] = size(tim_hist);
               hold on;
               yyaxis right;
               ylim([0 40])
    ...
    cd([num2str(exp_day) '/' save_fold '/' monkey_name num2str(exp_day) '_' task '_' synergy_type '/' monkey_name num2str(exp_day) '_syn_result_' sprintf('%02d', EMG_num) '/' monkey_name num2str(exp_day) '_H']);
