...
matlabver   = matlabver.Version;
ind =strfind(matlabver,'.');
if(length(ind)==1)
    ind     = [ind,(length(matlabver)+1)];
end
matlabver   = matlabver(1:ind(2)-1);
matlabver   = str2double(matlabver);

if(matlabver<=7.1)
    if(isempty(varargin))
        hL = plot(ax, x, y);
    else
        hL = plot(ax, x, y, varargin{:});
    end
    return;
end
...
% find indeces inside the range plus one
idrng(1)    = max(find(UD.x>=rng(1),1,'first')-1,1);
idrng(2)    = min(find(UD.x<=rng(2),1,'last')+1,length(UD.x));
id          = idrng(1):idrng(2);
