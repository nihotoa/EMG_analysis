function [Y,Y_dat]  = makeEMGNMFUchida(X,kf,nrep,nshuffle,alg)

% [Y,Y_dat]  = makeEMGNMF(X,kf,nrep,nshuffle,alg);
% 
% kf  :  k-fold cross-validation????(k) ?f?[?^??k?????Z?O?????g?????????A????????????test???g???A????train???g???B
% ?????????AY.train.r2 ??????k???????????????A???I?I???????????????????g???B
% kf=1?????????Across-validation???s???????B
% 
% ex
...
%         XX  = normalize(XX,'mean');
       [Y_dat.train.W{ii,ikf},Y_dat.train.H{ii,ikf},Y_dat.train.D{ii,ikf}]   = nnmf2(XX,ii,[],[],nrep,alg,'wh','none');
...
if(nshuffle>0)
% shuffle????train-test???s?????Atest???????f?[?^???????o???????B
...
%     Xvar    = var(reshape(Xshuf,numel(Xshuf),1),0);
    ...
    for ii=1:mm
        ...
        for ikf=1:kf
            ...
            XX  = Xshuf(:,~kfind(ikf,:));
%             XX  = normalize(XX,'mean');
            [W,H]   = nnmf2(XX,ii,[],[],nrep,alg,'wh','none');
