function cohtrig_btc(command,comment)
...
%         TimeRange   = [1 get(currExp,'TotalTime')]
        ...
        disp(['Cohtrig_btc: ',ExperimentName,' (',num2str(kk),'/',num2str(nExp),') start at ',datestr(now)]);
        ...
        Outputpath  = ['c:\data\cohtrig\',ExperimentName,'\'];
        ...
        %         maxaxes = 5;
                ...
                for ii=1:nCh
                    for jj=ii+1:nCh
                        [f, cl]=cohtrig(currExp, TimeRange, nfftPoints,list{ii},list{jj},trigName,trigWindow,opt_str);
                ...
                for ii=1:size(fig,1)
                    for jj=1:size(fig,2)
                        if(fig(ii,jj)~=0)
                            ...
                            uicontrol(fig(ii,jj),'Backgroundcolor',[1 1 1],'Style','text','Position',[10 10 500 30],'String',[get(fig(ii,jj),'Name'),' ( [',num2str(TimeRange(1)),'-',num2str(TimeRange(2)),'] seconds, ', num2str(cl.df) ,' Hz resolution, ', num2str(cl.seg_tot), ' segments, ', num2str(cl.seg_size), ' points)' ]);
...
%                             close(fig(ii,jj))
    ...
    %             maxaxes = 5;
                ...
                for ii=1:nCh
                    [f, cl]=cohtrig(currExp, TimeRange, nfftPoints,control{1},list{ii},trigName,trigWindow,opt_str);
                ...
                for ii=1:length(fig)
                    if(fig(ii)~=0)
                        ...
                        uicontrol(fig(ii),'Backgroundcolor',[1 1 1],'Style','text','Position',[10 10 500 20],'String',[get(fig(ii),'Name'),' ( [',num2str(TimeRange(1)),'-',num2str(TimeRange(2)),'] seconds, ', num2str(cl.df) ,' Hz resolution, ', num2str(cl.seg_tot), ' segments, ', num2str(cl.seg_size), ' points, ''', opt_str,''')']);
