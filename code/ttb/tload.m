function varargout  = tload(dlgtitle)
if nargin < 1
...
if(exist(configfile))
    ...
    if(isfield(myconfig,'tload'))
        lastdir = myconfig.tload.pathname;
        if(~exist(lastdir))
            lastdir = CurrDir;
        end
        lastfile  = myconfig.tload.filename;
    else
        lastdir = CurrDir;
        lastfile    = '*.*'
    end
...
elseif(class(filename) == 'cell')
    ...
    myconfig.tload.pathname = pathname;
    myconfig.tload.filename = filename{1};
    save(configfile,'myconfig');

else
    S   = open([pathname,'\',filename]);
    myconfig.tload.pathname = pathname;
    myconfig.tload.filename = filename;
    save(configfile,'myconfig');
