...
trig_2 = [1 4]; %1:plot obj1 start timing, 
                ...
                %3:plot obj2 start timing, 
                %4:plot obj2 end timing, 
range_normalize = 2000;%normalize range of task[msec]           
...
%         end
        ...
        if save_ave == 1
            ...
            save([monkeyname xpdate '_trig' sprintf('%d', trig) '.mat'], 'TimeRange','comment', 'All_ave', 'Unit');
            cd ../../
        end
    end
...
if plk == 2
    ...
    for j = 1:EMG_num
        ...
        for i=1:SUC_num
                subplot(4,3,j,'Parent',p); plot(x,pullData(i,:),'Color',[0,0,0]);
                hold on;
                ave = ((ave .* (i-1)) + pullData(i,:)) ./ i;
        end
        All_ave(:,j) = ave';
