...
for J =1:S(2)     %session loop
   cd([Allfiles{J} '_' task '_' taskALP '/' Allfiles{J} '_syn_result_09/' Allfiles{J} '_H/'])
   K = load([Allfiles{J} '_aveH3_4.mat'],'k');
   cd ../../
%    TimSUC = load([Allfiles{J} '_SUC_Timing.mat']);
   synergyData = load([Allfiles{J} '_' task '_' taskALP '_09_nmf.mat'],'test');
...
for SS = 1:S(2) %session loop
   ...
   if save_data == 1
       ...
       mkdir 'synData';
       cd 'synData'
       save([monkeyname 'Synfixed_' taskALP '_' AllDays{SS} '_Pdata.mat'], 'monkeyname','xpdate','D',...
