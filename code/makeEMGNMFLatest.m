...
%
% nrep:   optimaization??LocalMinima???????????????????????J?????????A?????????????????l???g???i???A10?j
% nshuffle:   ???f?[?^?V???b?t???????f?[?^?????????????s?????????????J???????????w???i???A1?j
        ...
        for ii=1:mm
            ...
            if(ikf>1)
                ...
                for ikf=1:kf
                    ...
                    XX  = Xshuf(:,~kfind(ikf,:));
                    %             XX  = normalize(XX,'mean');
                    [W,H]   = nnmf2(XX,ii,[],[],nrep,alg,'wh','mean','mean');
                    Y_dat.shuffle.W{ii,ikf,ishuffle}    = W;
                    
                    % test
                    XX  = Xshuf(:,kfind(ikf,:));
                    %             XX  = normalize(XX,'mean');
                    [W,H]   = nnmf2(XX,ii,W,[],nrep,alg,'h','mean','none');
