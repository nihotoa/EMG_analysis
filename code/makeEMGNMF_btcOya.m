...
%{
【注意事項】
TimeRange2の用途が不明だったため、コメントアウトした
offsetではなくoffset2を使用
【Nibali解析の場合】
pre : untitled.m
post : makefold
【やちむん解析の場合】
...
if nargin<1
    TimeRange   = [0 Inf];
    kf          = 4; %ここで、クロスバリデーションを行うかどうか、行う場合は何分割するかを設定できる
    nrep        = 20;
    nshuffle    = 1;
...
disp('【Plese select used EMG Data')
...
disp('【Plese select the dir to save generated Synergy Data')
...
%         X(:,1:SampleRate)   = [];
        ...
        %整流→平滑化だと、最小値がマイナスの値をとるので、OFFSETして対処する
        X   = offset(X,'min');
