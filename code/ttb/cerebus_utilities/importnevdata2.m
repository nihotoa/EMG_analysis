    ...
    for iID=1:nID
        ...
        try
            if(EntityInfo(ID).EntityType==1)
                ...
                switch(nsEventInfo.EventType)
                    case 'ns_EVENT_WORD'
                        ...
                        for iData=1:nData
                            binData(:,iData)    = logical(dec2binvec(Data(iData)));
                        end
                                                
                        for iCh=1:nCh
                            S.Name          = [Name,num2str(iCh,'%.2d')];
                            S.Data  = round(TimeStamp(binData(iCh,:)==1)'*SampleRate);
            ...
            elseif(EntityInfo(ID).EntityType==4)
                disp([Name ': Neural Entity Under construction'])
