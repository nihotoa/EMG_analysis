...
%{
注意点
これ回す前にdisp_NMF_WとshareTimperを回す
・疑問点
order_tim_listの中にあるフォルダはどこで作られる？
図は保存されたが、データの保存のセクションはなかった
%}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
clear
%% settings
...
%          c = c1(5:50,:);
        ...
        %↓please conduct dispNMF_W first!!
        cd order_tim_list
        cd([monkeyname mat2str(days(1)) 'to' mat2str(days(end)) '_' sprintf('%d',Ld)])
            load([monkeyname mat2str(days(1)) 'to' mat2str(days(end)) '_' sprintf('%d',Ld) '_' sprintf('%d',syn_num) '.mat'],'k_arr');
            load Tim_per.mat;
