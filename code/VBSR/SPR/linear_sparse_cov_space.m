function [Model, Info] = linear_sparse_cov_space(X,Y,Model,parm)
...
%     Sparse constraint is not imposed into time embedding dimension
%
...
%  if Model is previous training result, re-training is done
%
...
if length(SY) == 1 && N > 1, SY = repmat(SY,N,1) ; end;
...
for k=1:Ntrain
    ...
    % Weight variance
	SW  = XX + diag(1./A);
    if ~isfinite(rcond(SW)),
    	fprintf('Singular SW\n')
    	break
    elseif rcond(SW) > MinCond,
		SW  = inv( SW );
	else
		SW  = pinv( SW );
	end
	...
	if k <= Npre_train,
		...
		%	A  = (1./SY)' * (W.^2) ./ (G_A * N);	
	    A  = (((1./SY)' * WW) + 2*Ta0*a0)./(G_A * N + 2*Ta0);
	end
	
	% average time delay components in A
	Amean = mean(reshape(A,[M/D,D]),2);
	Amean = repmat(Amean, 1, D);
	A = Amean(:)';
    ...
    if Prune == 1
		...
		A_all  = zeros(1,M_all); 
		A_all(ix_act) = sum(WW,1)/max(sum(WW,1));
	...
	if M==0, 
		fprintf('All components are deleted\n')
		break
	end
end
