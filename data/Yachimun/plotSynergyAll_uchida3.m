function plotSynergyAll_uchida3(fold_name,emg_group,pcNum,plk)
...
%% plot T (pullData contain 50% time data before obj1 hold_on)
...
if plk == 1
    ...
    TIME_W = 150;
    ave = zeros(pcNum,TIME_W*2);
    pullData = zeros(SUC_num,TIME_W*2);
    sec = zeros(3,1);
    ...
    for j=1:pcNum
        for i=1:SUC_num
           ...
           if time_w == TIME_W
               sec(1,1) = sec(1,1)+1;
               if filt_on == 1
                   pullData(i,:) = filtfilt(bL,aL, abs(filtfilt(bH,aH, All_T(j,SUC_Timing_A(i,1)-floor((SUC_Timing_A(i,4)-SUC_Timing_A(i,1))):SUC_Timing_A(i,4)))));
               else
                   pullData(i,:) = All_T(j,SUC_Timing_A(i,1)-floor((SUC_Timing_A(i,4)-SUC_Timing_A(i,1))):SUC_Timing_A(i,4));
               end
           elseif time_w<TIME_W 
               sec(2,1) = sec(2,1)+1;
               if filt_on == 1
               pullData(i,:) = filtfilt(bL,aL, abs(filtfilt(bH,aH, interpft(All_T(j,SUC_Timing_A(i,1)-floor((SUC_Timing_A(i,4)-SUC_Timing_A(i,1))):SUC_Timing_A(i,4)),TIME_W*2))));
               else
                   pullData(i,:) = interpft(All_T(j,SUC_Timing_A(i,1)-floor((SUC_Timing_A(i,4)-SUC_Timing_A(i,1))):SUC_Timing_A(i,4)),TIME_W*2);
               end
           else
               sec(3,1) = sec(3,1)+1;
               if filt_on == 1
                   pullData(i,:) = filtfilt(bL,aL, abs(filtfilt(bH,aH, resample(All_T(j,SUC_Timing_A(i,1)-floor((SUC_Timing_A(i,4)-SUC_Timing_A(i,1))):SUC_Timing_A(i,4)),TIME_W*2,floor(time_w*2)))));
               else
                   pullData(i,:) = resample(All_T(j,SUC_Timing_A(i,1)-floor((SUC_Timing_A(i,4)-SUC_Timing_A(i,1)+1)):SUC_Timing_A(i,4)),TIME_W*2,floor(time_w*2));
               end
           end
           plot(pullData(i,:));
