...
switch Tar
   case 'EMG'
      ...
      %    eval(['Re.' EMGs{i,1} '{1,i}= zeros(s,68);']);
         for k = 1:TarN
             eval(['Re.' EMGs{i,1} '{1,k}= zeros(68,1);']);
             for j = 1:68
...
switch Tar
   case 'EMG'
      DD = load('AllDataforXcorr_68.mat');
      ...
      conD = cell(1,14);
      conD{1} = mean(D.Delt(:,1:8),2);
      conD{2} = mean(D.Biceps(:,1:8),2);
      conD{3} = mean(D.Triceps(:,1:8),2);
      conD{4} = mean(D.BRD(:,1:8),2);
      conD{5} = mean(D.cuff(:,1:8),2);
      conD{6} = mean(D.ED23(:,1:8),2);
      conD{7} = mean(D.ED45(:,1:8),2);
      conD{8} = mean(D.ECR(:,1:8),2);
      conD{9} = mean(D.ECU(:,1:8),2);
      conD{10} = mean(D.EDC(:,1:8),2);
      conD{11} = mean(D.FDS(:,1:8),2);
      conD{12} = mean(D.FDP(:,1:8),2);
      conD{13} = mean(D.FCU(:,1:8),2);
      conD{14} = mean(D.FCR(:,1:8),2);
...
DD = load('DataSetForEachXcorr_68_Range2.mat');
