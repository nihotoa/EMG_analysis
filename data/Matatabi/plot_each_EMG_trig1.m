...
%                     ylim([0 80]);
                    if save_fig == 1
                     cd EMG_plot_figures;
                     saveas(gcf,[ monkeyname xpdate '-' sprintf('%d', file_num(1)) '-trig1-' EMGs{j} '-' sprintf('%d',i ) '-' sprintf('%d', filt_h) 'HzHP-rect-' sprintf('%d', filt_l) 'HzLP-100Hz.bmp']);
                     cd ../;
                    end
                    hold on;
...
%     SUC_num = 142;
    ...
    for j = 1:EMG_num   
        ...
        if j == 3 || j == 6 || j==7 || j==8
            plotset =[1 1 1 2 2 2 3 4];
        for i=1:SUC_num
                subplot(2,2,plotset(j),'Parent',p); plot(x,all_pullData(i,:),'Color',[0,0,0]);
                hold on;
        end
